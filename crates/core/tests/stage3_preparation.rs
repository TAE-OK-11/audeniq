...
use audeniq_core::{
    ...
    ern::{generate_prepared_ern, validate_xml},
    error::{Error, Result},
...
fn fixture(index: usize) -> (PreparedRelease, VerificationPackage, MemoryStore) {
    let raw = [
        include_str!("fixtures/ern/single.json"),
        include_str!("fixtures/ern/ep.json"),
        include_str!("fixtures/ern/album.json"),
    ][index];
    let mut c: PreparedRelease = serde_json::from_str(raw).unwrap();
    let body = json!({
    ...
    c.canonical.verification_package_hash = c.verification_package_hash.clone();
    let v = VerificationPackage {
        id: c.verification_package_id,
        org_id: c.org_id,
        revision_id: c.revision_id,
        rights_epoch: c.rights_epoch,
        package_hash: c.verification_package_hash.clone(),
        body,
    };
    let mut store = MemoryStore::default();
    store.files.insert(
        c.artwork.object_key.clone(),
        (
            b"synthetic-artwork".to_vec(),
            c.artwork.content_type.clone(),
        ),
    );
    for (i, t) in c.tracks.iter().enumerate() {
        store.files.insert(
            t.audio.object_key.clone(),
            (
                format!("synthetic-audio-{i}").into_bytes(),
                t.audio.content_type.clone(),
            ),
        );
    }
    (c, v, store)
...
fn facts(c: &PreparedRelease, xml: &str) -> (FreshnessPin, CurrentFacts) {
    let pin = FreshnessPin {
        revision_id: c.revision_id,
        verification_hash: c.verification_package_hash.clone(),
        snapshot_id: c.snapshot_id,
        rights_epoch: c.rights_epoch as u64,
        route_contract_id: Uuid::from_u128(99),
...
impl ObjectStore for MemoryStore {
    ...
    async fn head(&self, key: &str) -> Result<Option<ObjectMeta>> {
        ...
        Ok(self.files.get(key).map(|(b, m)| ObjectMeta {
            size: b.len() as i64,
...
async fn three_partner_neutral_fixtures_pass_all_four_checks() {
    for i in 0..3 {
        ...
        let xml = generate_prepared_ern(&c).unwrap();
        let canonical_xml = audeniq_core::ern::generate_ern(&c.canonical).unwrap();
        assert_eq!(
            canonical_xml,
            audeniq_core::ern::generate_ern(&c.canonical).unwrap()
        );
        assert!(canonical_xml.contains("Synthetic Composer"));
        assert_eq!(xml, generate_prepared_ern(&c).unwrap());
        ...
        for p in plan {
            ...
            assert_eq!(p.audio_asset_ids.len(), c.tracks.len());
        }
        if let Ok(dir) = std::env::var("F4_FIXTURE_XML_DIR") {
            std::fs::create_dir_all(&dir).unwrap();
            std::fs::write(
                std::path::Path::new(&dir).join(format!("fixture-canonical-{i}.xml")),
                canonical_xml,
            )
            .unwrap();
            std::fs::write(
                std::path::Path::new(&dir).join(format!("fixture-{i}.xml")),
                xml,
            )
            .unwrap();
        }
...
fn xml_escaping_and_deterministic_order() {
    ...
    assert!(xml.contains("&apos;"));
    c.tracks.reverse();
    c.approved_scope.reverse();
    assert_eq!(xml, generate_prepared_ern(&c).unwrap());
    c.title.push('\0');
    assert!(generate_prepared_ern(&c).is_err());
}
...
async fn xml_and_metadata_fail_closed() {
    ...
    for bad in [
        xml.replace("</Release>", ""),
        xml.replace("<UPC>", "<Unknown>"),
        xml.replace("<Title>", "<Title>altered"),
        format!("<!DOCTYPE x [<!ENTITY x SYSTEM 'file:///etc/passwd'>]>{xml}"),
    ] {
        assert!(validate_xml(&c, &bad).is_err());
        assert_eq!(
            preflight(&c, &v, &bad, &pin, &current, &store).await.xml,
            CheckStatus::Fail
        );
    }
    c.title = "   ".into();
    let report = preflight(&c, &v, &xml, &pin, &current, &store).await;
    assert_eq!(report.metadata, CheckStatus::Fail);
...
async fn missing_corrupt_and_unavailable_files_never_pass() {
    ...
    assert_eq!(
        preflight(&c, &v, &xml, &pin, &current, &store).await.files,
        CheckStatus::Fail
    );
    let (_, _, mut store) = fixture(0);
    store
        .files
        .get_mut(&c.tracks[0].audio.object_key)
        .unwrap()
        .0[0] ^= 1;
    assert_eq!(
        preflight(&c, &v, &xml, &pin, &current, &store).await.files,
    ...
    let report = preflight(&c, &v, &xml, &pin, &current, &store).await;
    assert_eq!(report.files, CheckStatus::Unknown);
    assert!(!report.passed());
}
...
async fn freshness_and_approval_mismatch_never_pass() {
    for mutation in 0..9 {
        let (mut c, mut v, store) = fixture(0);
        let xml = generate_prepared_ern(&c).unwrap();
        ...
        assert_eq!(
            preflight(&c, &v, &xml, &pin, &current, &store).await.rights,
            CheckStatus::Fail,
            "mutation {mutation}"
        );
    }
...
fn empty_approval_creates_no_routes_and_duplicate_approval_is_rejected() {
    ...
    c.approved_scope.clear();
    c.canonical.approved_dsp_ids.clear();
    v.body["approved_scope"]["dsp_ids"] = json!([]);
    v.package_hash = digest(&v.body);
    c.verification_package_hash = v.package_hash.clone();
    c.canonical.verification_package_hash = v.package_hash.clone();
    assert!(plan_submissions(&c, &v).unwrap().is_empty());
    let (mut c, mut v, _) = fixture(0);
    v.body["approved_scope"]["dsp_ids"] =
        json!([c.approved_scope[0].dsp_id, c.approved_scope[0].dsp_id]);
    v.package_hash = digest(&v.body);
    c.verification_package_hash = v.package_hash.clone();
    c.canonical.verification_package_hash = v.package_hash.clone();
...
#[test]
fn muse_snapshot_missing_identifier_or_changed_pin_is_rejected() {
    let (mut p, _, _) = fixture(0);
    p.canonical.tracks[0].isrc = None;
    assert!(audeniq_core::ern::generate_ern(&p.canonical).is_err());
    assert!(generate_prepared_ern(&p).is_err());
    let (mut p, _, _) = fixture(0);
    p.canonical.tracks[0].asset_sha256 = Some("f".repeat(64));
    assert!(generate_prepared_ern(&p).is_err());
}
