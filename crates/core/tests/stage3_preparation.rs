...
async fn three_partner_neutral_fixtures_pass_all_four_checks() {
    for i in 0..3 {
        ...
        assert_eq!(canonical_xml, audeniq_core::ern::generate_ern(&c.canonical).unwrap());
        ...
        if let Ok(dir) = std::env::var("F4_FIXTURE_XML_DIR") {
            ...
            std::fs::write(std::path::Path::new(&dir).join(format!("fixture-canonical-{i}.xml")),canonical_xml).unwrap();
...
fn xml_escaping_and_deterministic_order() {
    ...
    assert_eq!(xml,generate_prepared_ern(&c).unwrap());
    c.title.push('\0'); assert!(generate_prepared_ern(&c).is_err());
}
...
async fn missing_corrupt_and_unavailable_files_never_pass() {
    ...
    let xml = generate_prepared_ern(&c).unwrap(); let (pin,current) = facts(&c,&xml);
...
async fn freshness_and_approval_mismatch_never_pass() {
    for mutation in 0..9 {
        let (mut c,mut v,store)=fixture(0); let xml=generate_prepared_ern(&c).unwrap();
...
fn empty_approval_creates_no_routes_and_duplicate_approval_is_rejected() {
    ...
    c.approved_scope.clear(); c.canonical.approved_dsp_ids.clear(); v.body["approved_scope"]["dsp_ids"]=json!([]);
    v.package_hash=digest(&v.body); c.verification_package_hash=v.package_hash.clone(); c.canonical.verification_package_hash=v.package_hash.clone();
