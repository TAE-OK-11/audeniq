...
use crate::{
    error::{Error, Result},
    preparation_model::{PreparedRelease, DspScope, VerificationPackage},
};
...
pub fn approved_dsps(v: &VerificationPackage) -> Result<BTreeSet<Uuid>> {
    ...
    if crate::domain::digest(b) != v.package_hash
        || b["decision"] != "PASS"
        || b["revision_id"].as_str().and_then(|s| Uuid::parse_str(s).ok()) != Some(v.revision_id)
        || b["rights_epoch"].as_i64() != Some(v.rights_epoch)
        || v.rights_epoch < 0
        || b["schema_version"].as_u64() != Some(1)
    { return Err(Error::Invalid); }
    let raw = b["approved_scope"]["dsp_ids"].as_array().ok_or(Error::Invalid)?;
    let mut ids = BTreeSet::new();
    for id in raw {
        let id = id.as_str().and_then(|s| Uuid::parse_str(s).ok()).ok_or(Error::Invalid)?;
        if id.is_nil() || !ids.insert(id) { return Err(Error::Invalid); }
    }
    Ok(ids)
}
...
pub fn verify_scope(c: &PreparedRelease, v: &VerificationPackage) -> Result<()> {
    if c.org_id != v.org_id || c.verification_package_id != v.id
        || c.revision_id != v.revision_id || c.rights_epoch != v.rights_epoch
    ...
    if scope.len() != c.approved_scope.len() || scope != approved { return Err(Error::Conflict); }
    Ok(())
}
...
pub fn plan_submissions(c: &PreparedRelease, v: &VerificationPackage) -> Result<Vec<SubmissionItems>> {
    verify_scope(c, v)?;
    crate::ern::validate_metadata(c)?;
    let audio_asset_ids = crate::ern::ordered_tracks(c).iter().map(|t| t.audio.id).collect::<Vec<_>>();
    Ok(approved_dsps(v)?.into_iter().map(|dsp_id| SubmissionItems {
