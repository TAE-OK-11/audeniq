...
pub fn validate_upc(value: &str) -> Result<()> {
    ...
    let sum: u32 = b.iter().enumerate().map(|(i, c)| {
        u32::from(c - b'0') * if i % 2 == 0 { 3 } else { 1 }
    }).sum();
...
impl IdentifierKind {
    fn label(self) -> &'static str {
        match self { Self::Isrc => "ISRC", Self::Upc => "UPC" }
    }

    fn validate(self, value: &str) -> Result<()> {
        match self { Self::Isrc => validate_isrc(value), Self::Upc => validate_upc(value) }
    }
}
...
pub async fn record_existing(c: &mut PgConnection, a: &ExistingAssignment<'_>) -> Result<Uuid> {
    ...
    if !belongs { return Err(Error::Conflict); }
    let id = Uuid::new_v4();
...
mod tests {
    ...
    fn normalized_isrc_only() {
        for s in ["USAAA2600001", "KRA1Z2600002"] { assert!(validate_isrc(s).is_ok()); }
        for s in ["", "US-AAA-26-00001", "usAAA2600001", "1SAAA2600001", "USAAA260000１", " USAAA2600001", "USAAA2600001\n"] {
            assert!(validate_isrc(s).is_err(), "{s:?}");
        }
    }
    #[test]
    fn upc_checksum_and_leading_zero() {
        for s in ["012345678905", "036000291452"] { assert!(validate_upc(s).is_ok()); }
        for s in ["", "012345678904", "0036000291452", "03600029145", "000000000000", "03600029145X", "036000291452\n"] {
    ...
    fn issuance_cannot_be_enabled() {
        for k in [IdentifierKind::Isrc, IdentifierKind::Upc] {
            assert!(matches!(issue_identifier(k), Err(Error::PolicyGate("IDENTIFIER_ISSUANCE_OFF"))));
        }
    }
}
