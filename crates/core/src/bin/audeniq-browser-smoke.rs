...
struct Browser { http: reqwest::Client, base: String }
impl Browser {
    async fn post(&self, path: &str, value: Value) -> anyhow::Result<Value> {
        let r: Value=self.http.post(format!("{}{path}",self.base)).json(&value).send().await?.error_for_status()?.json().await?;
        anyhow::ensure!(r["value"]["error"].is_null(),"WebDriver command failed: {}",r["value"]["error"]);
        Ok(r["value"].clone())
    }
    async fn find(&self, selector: &str) -> anyhow::Result<String> {
        for _ in 0..60 {
            if let Ok(v)=self.post("/element",json!({"using":"css selector","value":selector})).await {
                if let Some(id)=v["element-6066-11e4-a52e-4f735466cecf"].as_str(){return Ok(id.into());}
            }
    ...
    async fn type_in(&self, selector: &str, text: &str) -> anyhow::Result<()> {
        let id=self.find(selector).await?;self.post(&format!("/element/{id}/clear"),json!({})).await?;
        self.post(&format!("/element/{id}/value"),json!({"text":text})).await?;Ok(())
    }
    async fn click(&self, selector: &str) -> anyhow::Result<()> {
        let id=self.find(selector).await?;self.post(&format!("/element/{id}/click"),json!({})).await?;
    ...
    async fn assert_text(&self, value: &str) -> anyhow::Result<()> {
        let text=self.post("/execute/sync",json!({"script":"return document.body.innerText","args":[]})).await?;
        anyhow::ensure!(text.as_str().unwrap_or_default().contains(value),"expected UI text: {value}; actual: {text}");Ok(())
    }
}
#[tokio::main]
async fn main() -> anyhow::Result<()> {
    ...
    let id=v["value"]["sessionId"].as_str().ok_or_else(||anyhow::anyhow!("no browser session"))?;
    let browser=Browser {http,base:format!("http://127.0.0.1:9515/session/{id}")};
    let result=run(&browser).await;
    browser.http.delete(&browser.base).send().await?;
    result
}
async fn run(b: &Browser) -> anyhow::Result<()> {
    b.post("/url",json!({"url":"http://localhost:5173"})).await?;
    b.type_in("#email",&format!("browser-{}@example.test",uuid::Uuid::new_v4())).await?;
    ...
    println!("Browser smoke passed: registration, session, artist/release writes, escaped metadata, preflight gate, reload CSRF recovery and logout");
    Ok(())
}
