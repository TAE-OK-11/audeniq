...
use crate::{contracts::DistributionPackage, error::{Error, Result}};
...
pub async fn store_package(c: &mut PgConnection, p: &DistributionPackage) -> Result<Uuid> {
    ...
    if !valid_hash(&p.package_digest) || !valid_hash(&p.manifest_hash) || p.id.is_nil() || p.immutable_bytes_ref.is_nil() {
