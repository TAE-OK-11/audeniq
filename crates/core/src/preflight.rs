...
pub enum CheckStatus { Pass, Fail, Unknown }
...
impl PreflightReport {
    pub fn passed(&self) -> bool {
        [self.xml, self.metadata, self.files, self.rights].iter().all(|s| *s == CheckStatus::Pass)
    }
}

fn status(pass: bool) -> CheckStatus { if pass { CheckStatus::Pass } else { CheckStatus::Fail } }
...
pub async fn check_files(c: &CanonicalRelease, store: &dyn ObjectStore) -> CheckStatus {
    if validate_metadata(c).is_err() { return CheckStatus::Fail; }
    let mut outcome = CheckStatus::Pass;
    for a in std::iter::once(&c.artwork).chain(c.tracks.iter().map(|t| &t.audio)) {
        if a.size_bytes > MAX_PREFLIGHT_ASSET_BYTES { return CheckStatus::Fail; }
        match store.head(&a.object_key).await {
            ...
            Err(_) => { outcome = CheckStatus::Unknown; continue; }
        }
...
pub fn check_rights(c: &CanonicalRelease, v: &VerificationPackage, xml: &str,
    expected: &FreshnessPin, current: &CurrentFacts) -> bool {
    verify_scope(c, v).is_ok()
        ...
        && freshness_guard(expected, &current.pin, current.hold, current.contract_active).is_ok()
}

pub async fn preflight(c: &CanonicalRelease, v: &VerificationPackage, xml: &str,
    expected: &FreshnessPin, current: &CurrentFacts, store: &dyn ObjectStore) -> PreflightReport {
    PreflightReport {
