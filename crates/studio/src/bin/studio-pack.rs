...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for part in source.split("<style>").skip(1) {
        if let Some((style, _)) = part.split_once("</style>") { css.push_str(style); css.push('\n'); }
    }
    if css.is_empty() { return Err("original Studio CSS missing".into()); }
    ...
    fs::copy("web/studio/connected/index.html", out.join("index.html"))?;
    fs::copy("web/studio/public/assets/AUDENIQ_Logo_Light.svg",out.join("assets/AUDENIQ_Logo_Light.svg"))?;
    // Minimal module bootstrap. All application state, requests and DOM handling are Rust.
    fs::write(out.join("boot.js"), "import init from './pkg/audeniq_studio.js';\ninit();\n")?;
