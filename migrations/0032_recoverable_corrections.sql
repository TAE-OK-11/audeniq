-- Sandbox round 2: every terminal pipeline failure must land in a visible,
-- recoverable correction state. A release in any *_CORRECTION status is
-- editable (tracks, credits, metadata) and can be resubmitted, which starts
-- a new application revision from Stage 1. Submitted content stays frozen in
-- catalog.application_revisions, so edits never alter what was reviewed.
INSERT INTO operations.allowed_transitions VALUES ('application_pipeline_status','STAGE3_CORRECTION','SUBMITTED')
 ON CONFLICT DO NOTHING;

CREATE OR REPLACE FUNCTION catalog.is_editable_status(s text) RETURNS boolean
 LANGUAGE sql IMMUTABLE AS $$
 SELECT s IN ('DRAFT','STAGE1_CORRECTION','STAGE2_CORRECTION','STAGE3_CORRECTION')
$$;
...
BEGIN
 ...
 IF parent_status IS NULL OR NOT catalog.is_editable_status(parent_status) OR parent_archived IS NOT NULL THEN
...
BEGIN
 ...
 IF parent_status IS NULL OR NOT catalog.is_editable_status(parent_status) OR parent_archived IS NOT NULL OR track_archived IS NOT NULL THEN
