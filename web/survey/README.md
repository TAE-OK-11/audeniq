...
## 화면: React + TypeScript (V6)

설문 화면은 `app/`의 React + TypeScript 앱이에요. 기존 정적 `index.html`·`app.js`·`motion.js`·`style.css`를 그대로 옮겨서 **마크업·클래스·문구·CSS·애니메이션·제출 내용이 이전과 같아요** (단계별 DOM과 화면 픽셀, 제출 JSON을 원본과 비교해 확인).

| 파일 | 역할 |
|---|---|
| `app/src/survey.ts` | 문항·단계 데이터 (기존 HTML 문구 그대로, `questions.json`과 제목·선택지 일치를 테스트로 확인) |
| `app/src/logic.ts` | 검증·배타 선택·최대 개수·경험 분기·제출 본문 (기존 `app.js` 규칙) |
| `app/src/App.tsx` | 화면 |
| `app/src/motion.ts` | 등장 애니메이션 (기존 `motion.js`) |
| `app/src/style.css` | 기존 CSS 그대로 |
| `app/public/` | 로고·파비콘·robots.txt·`_headers` (빌드 때 그대로 복사) |

`bun run build`가 `app/`을 빌드해 **`public/`에 정적 파일로** 내보내요. 배포는 지금처럼 정적 자산 우선이라 페이지 조회에 Worker가 실행되지 않아요. `bun run deploy`는 빌드 → preflight → 배포 순서로 돌아요. 로컬 화면 개발은 `cd app && bun run dev`.
...
정적 요청용 보안 헤더는 `app/public/_headers`에서 설정했습니다 (빌드 때 `public/_headers`로 복사). `html_handling=auto-trailing-slash`로 `/`에 `public/index.html`을 제공하고, `not_found_handling=404-page`로 없는 탐색 주소도 정적 404를 제공합니다. `/api/*` 요청은 정적 응답으로 빠지지 않도록 Worker-first로 지정했습니다.
...
curl -sS -o /dev/null -w 'favicon HTTP %{http_code}\n' https://survey.audeniq.com/assets/favicon.svg
...
- 애니메이션은 `app/src/motion.ts`(빌드 결과 `public/assets/*.js`)에서 **방문자의 브라우저**에서만 실행. Worker 코드/CPU와 D1 쿼리를 추가하지 않음.
