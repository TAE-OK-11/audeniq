/* AUDENIQ Survey motion — matched to the public homepage's reveal timing.
...
function enter(el, delay = 0, distance = 16, duration = 760) {
...
export function initializeMotion() {
  ...
  try {
    ...
    scheduleStep(document.querySelector('.step:not([hidden])'));
  } catch { revealAll(); }
  reduced.addEventListener?.('change', () => { if (reduced.matches) revealAll(); });
  document.addEventListener('focusin', ({target}) => {
    for (const el of [...pending]) if (el.contains(target)) reveal(el, true);
    for (const el of [...active.keys()]) if (el.contains(target)) settle(el);
  });
...
export function animateSurveyStep(step) {
...
export function animateSurveyComplete(complete) {
  revealAll();
